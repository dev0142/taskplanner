...
function TaskDetails(props) {
  ...
  const [assigneeName, setAssigneeName] = useState(props.specificTask.assignee);
  ...
  const handleAssigneeChange = async (e, id, assignee, assigneeName) => {
    e.preventDefault();
    let res = await fetch(
      `${process.env.NEXT_PUBLIC_host}` +
        "/api/" +
        `${id}` +
        "?source=assigneeChange",
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const [allEmployeeNamesAndId, setAllEmployeeNamesAndId] = useState([]);
  const getAllEmployees = async () => {
    let employeeNamesAndId = [];
    ...
    employees.map((item, index) => {
      ...
      employeeNamesAndId.push(employeeDetails);
    });
    setAllEmployeeNamesAndId(employeeNamesAndId);
  };
  useEffect(() => {
    getAllEmployees();
  }, []);
  ...
  return (
    <>
      ...
      <Box sx={{ width: "100%" }}>
        ...
        >
          <Box sx={{ width: { xs: "95%", md: "35%" }, margin: {xs:"10px 0px",md:"10px"} }}>
            <Box
              sx={{
                height: "auto",
                borderRadius: "7px",
          ...
          <Box sx={{  width: { xs: "95%", md: "35%" },margin: {xs:"10px 0px",md:"10px"}}}>
            ...
            >
              ...
              <TextField
                ...
                value={assigneeName}
                fullWidth
                helperText={!assigneeName ? "Not assigned to anybody" : ""}
                onChange={(e) => {
                  setAssigneeName(e.target.value);
                  handleAssigneeChange(
                    e,
                    props.specificTask._id,
                    e.target.value,
                    event.target.getAttribute("name")
                  );
                }}
              >
                {allEmployeeNamesAndId.map((item, index) => {
                  return (
                    <MenuItem
                      key={index}
          ...
          <Box sx={{ width: { xs: "95%", md: "20%" }, margin: {xs:"10px 0px",md:"10px"} }}>
            <Box
              sx={{
                height: "auto",
                borderRadius: "7px",
            ...
            >
              <Button
                ...
                onClick={(e) => {
                  handleDeleteTask(e, props.specificTask._id);
                }}
